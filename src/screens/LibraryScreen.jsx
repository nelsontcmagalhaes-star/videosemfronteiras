...
import { Play, Download, Trash2, RefreshCw, BookOpen, Globe, Loader, X } from 'lucide-react'
...
function VideoModal({ video, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="card w-full max-w-sm p-5 fade-in" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-sm truncate pr-2" style={{ color: C.text }}>{video.name}</h3>
        ...
        {video.downloadUrl ? (
          <>
            ...
            <a
              ...
              download={`${video.name?.replace(/\.[^.]+$/, '') || 'video'}_dublado.mp4`}
          ...
          <div className="text-center py-6">
            <Globe size={36} color={C.faint} className="mx-auto mb-3"/>
            <p className="text-sm" style={{ color: C.muted }}>
              Este vídeo foi traduzido em outra sessão.<br/>
              Traduza novamente para obter o download.
            </p>
          </div>
        )}
      </div>
  ...
  if (!library.length) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center fade-in">
        ...
        <button onClick={onRetranslate} className="btn-primary mt-6 px-6 py-3 text-sm">
          Traduzir um vídeo
        </button>
      </div>
    )
          ...
          <div key={v.id} className="card p-4 fade-in">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold"
                    style={{ background: 'rgba(42,171,142,0.12)', color: C.dark }}>
                    {v.downloadUrl ? '✅ Pronto' : '📁 Registrado'}
                  </span>
                ...
                <div className="flex gap-3 mt-1 text-xs" style={{ color: C.faint }}>
                  <span>📅 {v.date}</span>
                  {v.duration && v.duration !== '--:--' && <span>⏱ {v.duration}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-3 pt-3" style={{ borderTop: `1px solid rgba(42,171,142,0.12)` }}>
              ...
              {v.downloadUrl ? (
                <a
                  href={v.downloadUrl}
                  download={`${v.name?.replace(/\.[^.]+$/, '') || 'video'}_dublado.mp4`}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all"
                  style={{ background: `linear-gradient(135deg,${C.primary},${C.dark})`, color: 'white', textDecoration: 'none' }}>
